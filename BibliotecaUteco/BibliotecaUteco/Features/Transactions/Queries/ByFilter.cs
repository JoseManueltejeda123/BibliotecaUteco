...
public class GetTransactionsByFilterCommand : ICommand<IApiResult>
{
    [
        ...
        Description("El nombre del usuario que hizo la transacción"),
        MaxLength(20)
    ]
    ...
    [
        ...
        Description("Cantidad de transacciones a omitir"),
    ...
    [
        ...
        Description("Cantidad de transacciones a tomar"),
        Range(1, 10)
    ]
    public int? Take { get; set; } = 10;
}

public class GetTransactionsByFilterCommandValidator
    : AbstractValidator<GetTransactionsByFilterCommand>
{
    public GetTransactionsByFilterCommandValidator()
    {
        RuleFor(x => x.UserName)
            ...
            .WithMessage(
                "El nombre de usuario para filtrar las transacciones debe de ser mayor a cero. "
            );

        RuleFor(x => x.Skip)
            .GreaterThanOrEqualTo(0)
            .When(x => x.Skip.HasValue)
            .WithMessage("La cantidad de transacciones a omitir debe de ser mayor o igual a 0 ");

        RuleFor(x => x.Take)
            .InclusiveBetween(1, 10)
            .When(x => x.Take.HasValue)
            .WithMessage("La cantidad de transacciones a tomar debe de ser entre 1 y 10");
    }
}
...
public class GetTransactionsByFilterEndpoint : IEndpoint
{
    ...
    {
        app.MapGet(
                EndpointSettings.TransactionsEndpoint + "/by-filter",
                async (
...
internal class GetTransactionsByFilterCommandHandler(IBibliotecaUtecoDbContext context)
    : ICommandHandler<GetTransactionsByFilterCommand, IApiResult>
{
    public async Task<IApiResult> HandleAsync(
        GetTransactionsByFilterCommand request,
        CancellationToken cancellationToken = default
    )
    {
        var result = await context.Transactions.GetByFilterAsync(
            request.UserName,
            request.Skip ?? 0,
            request.Take ?? 0,
            cancellationToken
        ...
        return new SuccessApiResult<List<TransactionResponse>>(
            result.Select(t => t.ToResponse()).ToList()
        );
    }
}
