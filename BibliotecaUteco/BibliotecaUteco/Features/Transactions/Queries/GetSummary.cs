        ...
        )
        {
            var summary = await context
                ...
                .Select(g => new CashBoxSummaryResponse
                {
                    ...
                    LastDepositDate = g.Where(t => t.Amount > 0)
                        .OrderByDescending(t => t.CreatedAt)
                        .Select(t => (DateTime?)t.CreatedAt)
                        .FirstOrDefault(),

                    LastRetirementAmount = g.Where(t => t.Amount < 0)
                        .OrderByDescending(t => t.CreatedAt)
                        .Select(t => Math.Abs(t.Amount))
                        .FirstOrDefault(),

                    LastRetirementDate = g.Where(t => t.Amount < 0)
                        .OrderByDescending(t => t.CreatedAt)
                        .Select(t => (DateTime?)t.CreatedAt)
                        .FirstOrDefault(),
                })
            ...
            if (summary == null)
            {
                return new NotFoundApiResult("No se encontraron transacciones en el sistema");
            }
