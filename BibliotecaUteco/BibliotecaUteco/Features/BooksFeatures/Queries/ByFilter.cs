namespace BibliotecaUteco.Features.BooksFeatures.Queries
{
    public class GetBooksByFilterCommand : ICommand<IApiResult>
    {
        [
            ...
            MaxLength(100),
            FromQuery(Name = "bookName"),
            Description("El nombre del libro a buscar")
        ]
        ...
        [
            ...
            Description("El nombre del genero del libro a buscar")
        ]
        ...
        [
            ...
            MaxLength(100),
            FromQuery(Name = "authorName"),
            Description("El nombre del autor del libro a buscar")
        ]
        ...
        [
            ...
            FromQuery(Name = "skip"),
            Description("Cantidad de libros a omitir")
        ]
    ...
    public class GetBooksByFilterCommandValidator : AbstractValidator<GetBooksByFilterCommand>
    {
        public GetBooksByFilterCommandValidator()
        {
            RuleFor(x => x.BookName)
                .MaximumLength(100)
                .WithMessage("El nombre del libro no puede superar los 100 caracteres")
                .When(x => !string.IsNullOrEmpty(x.BookName));

            RuleFor(x => x.GenreName)
                .MaximumLength(50)
                .WithMessage("El nombre del género no puede superar los 50 caracteres")
                .When(x => !string.IsNullOrEmpty(x.GenreName));

            RuleFor(x => x.AuthorName)
                .MaximumLength(100)
                .WithMessage("El nombre del autor no puede superar los 100 caracteres")
            ...
            RuleFor(x => x.Skip)
                ...
                .WithMessage("La cantidad de libros a omitir debe ser mayor o igual a 0");
        }
    }
    ...
    internal class GetBooksByFilterEndpoint : IEndpoint
    {
        ...
        {
            app.MapGet(
                    EndpointSettings.BooksEndpoint + "/by-filter",
                    async (
                ...
                .WithDescription(
                    $"Retorna una lista de 10 libros según el filtro dado {nameof(IApiResult)}"
                );
        }
    }

    public class GetGenresByNameHandler(IBibliotecaUtecoDbContext context)
        : ICommandHandler<GetBooksByFilterCommand, IApiResult>
    {
        public async Task<IApiResult> HandleAsync(
            GetBooksByFilterCommand request,
            CancellationToken cancellationToken = default
        )
        {
            var books = await context.Books.GetByFilterAsync(
                request.GenreName,
                request.BookName,
                request.AuthorName,
                request.Skip,
