...
{
    [
        FromQuery(Name = "identityCardNumber"),
    ...
    [
        FromQuery(Name = "studentLicence"),
        JsonPropertyName("studentLicence"),
        MaxLength(9),
        MinLength(3)
    ]
...
public class GetLoansByFilterValidator : AbstractValidator<GetLoansByFilter>
{
    public GetLoansByFilterValidator()
    {
        ...
        RuleFor(x => x.Skip).GreaterThanOrEqualTo(0).WithMessage("Skip debe ser mayor o igual a 0");

        RuleFor(x => x.Take).InclusiveBetween(1, 100).WithMessage("Take debe estar entre 1 y 100");

        RuleFor(x => x)
            .Must(x =>
            {
                var statusFiltersCount = new[]
                {
                    x.JustPendingOnes,
                    x.JustExceededOnes,
                    x.JustReturnedOnes,
            ...
            .WithMessage(
                "Solo puede seleccionar un tipo de estado a la vez (pendientes, excedidos o entregados)"
            );
    }
}
...
internal class GetLoansByFilterEndpoint : IEndpoint
{
    public void MapEndpoint(IEndpointRouteBuilder app)
    {
        app.MapGet(
            ...
            .Produces<SuccessApiResult<List<LoanResponse>>>(
                200,
...
public class GetLoansByFilterHandler(IBibliotecaUtecoDbContext context)
    : ICommandHandler<GetLoansByFilter, IApiResult>
{
    ...
    {
        var response = await context.Loans.GetByFilter(
        ...
        return new SuccessApiResult<List<LoanResponse>>(
            response.Select(l => l.ToResponse()).ToList()
        );
    }
}
