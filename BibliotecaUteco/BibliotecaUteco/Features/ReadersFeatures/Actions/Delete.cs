...
namespace BibliotecaUteco.Features.ReadersFeatures.Actions
{
    ...
    public class DeleteReaderCommandHandler(IBibliotecaUtecoDbContext context)
        : ICommandHandler<DeleteReaderCommand, IApiResult>
    {
        public async Task<IApiResult> HandleAsync(
            DeleteReaderCommand request,
            CancellationToken cancellationToken = default
        )
        {
            if (
            ...
            if (
                await context.Loans.AnyAsync(r => r.ReaderId == request.ReaderId, cancellationToken)
            )
            {
            ...
            if (
                await context.Loans.AnyAsync(
                    r => r.ReaderId == request.ReaderId && r.Penalty != null && r.Penalty.IsDue,
                    cancellationToken
                )
            )
            {
                return new BadRequestApiResult("Este lector tiene una penalizacion sin pagar");
            }

            var rows = await context
                .Readers.Where(r => r.Id == request.ReaderId)
                .ExecuteDeleteAsync();
            await context.SaveChangesAsync(cancellationToken);

            if (rows <= 0)
                return new BadRequestApiResult("No se eliminó ningun lector");
