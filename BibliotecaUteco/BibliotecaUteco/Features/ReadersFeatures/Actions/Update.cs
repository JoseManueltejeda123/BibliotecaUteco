...
namespace BibliotecaUteco.Features.ReadersFeatures.Actions
{
    public class UpdateReaderCommand : ICommand<IApiResult>
    {
        ...
        [MaxLength(
            9,
            ErrorMessage = "La matrícula estudiantil no puede tener más de 9 caracteres."
        )]
    ...
    public class UpdateReaderCommandHandler(IBibliotecaUtecoDbContext context)
        : ICommandHandler<UpdateReaderCommand, IApiResult>
    {
        public async Task<IApiResult> HandleAsync(
            UpdateReaderCommand request,
            CancellationToken cancellationToken = default
        )
        {
            if (
                await context.Readers.FirstOrDefaultAsync(
                    x => x.Id == request.ReaderId,
                    cancellationToken
                )
                    is var reader
                && reader is null
            ...
            if (!string.IsNullOrWhiteSpace(request.StudentLicence))
            {
                if (
                    await context.Readers.AnyAsync(
                        r => r.StudentLicence == request.StudentLicence && r.Id != request.ReaderId,
                        cancellationToken
                    )
                )
                {
            ...
            if (
                await context.Readers.AnyAsync(
                    r => r.PhoneNumber == request.PhoneNumber && r.Id != request.ReaderId,
                    cancellationToken
                )
            )
            {
            ...
            var updatedReader = await context.Readers.GetByIdAsync(
                request.ReaderId,
                cancellationToken
            );
